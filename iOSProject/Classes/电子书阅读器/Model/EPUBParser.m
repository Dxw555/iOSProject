...
-(NSString*)ncxFilePathWithOpfFile:(NSString*)opfFilePath WithUnzipFolder:(NSString*)unzipFolder
{
    ...
    {
        NSError *err = nil;
        GDataXMLDocument *opfXmlDoc = [[GDataXMLDocument alloc] initWithData:xmlData error:&err];
...
-(NSMutableArray*)epubCatalogWithNcxFile:(NSString*)ncxFilePath
{
    ...
    if (xmlData) {
        ...
        NSError *err = nil;
        GDataXMLDocument *ncxXmlDoc = [[GDataXMLDocument alloc] initWithData:xmlData error:&err];
        if ([err code] == 0)
        {
